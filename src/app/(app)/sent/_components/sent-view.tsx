...
import { SentCard, type SentEmail } from "./sent-card";
...
function groupEmails(emails: SentEmail[]): { key: string; emails: SentEmail[] }[] {
...
export function SentView({
  ...
  const t = useTranslations("pages.sent");
  ...
  const filtered = useMemo(() => {
    return emails.filter((e) => {
      ...
      const recipients = e.toAddresses
        ...
        <div className="bg-background md:border md:border-border md:rounded-2xl py-2">
          ...
          <div className="flex items-center justify-between px-[10px] md:px-4 py-3 mb-1">
            ...
            <span className="text-xs text-muted-foreground">
              {t("sentCount", { count: filtered.length })}
            </span>
          </div>
